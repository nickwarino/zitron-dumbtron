...
**Quote discipline.** The de-em-dash transform was at one point wrongly applied to verbatim quote fields. A 2026-06-12 restoration pass re-matched every Zitron quote in `claims-full.js` (464 restored), `categories.js` (438), and `comparisons.js` (37) against the local corpus in `crawl/md/` and restored the original punctuation, em dashes, curly quotes, and his own typos included. 26 claims-full quotes could not be re-matched (extraction drift, e.g. tense edits) and keep their stored text; treat those as paraphrase-grade. Markdown emphasis markers that the scan had captured raw are stripped (the renderer escapes, never formats). `scripts/validate.py` cannot police quote fields by design, so any future regeneration must re-run this corpus match.
